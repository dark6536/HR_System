...
<script type="text/javascript">
	...
	function myCalender(year, month, tbId) {
		...
		var maxIndex = new Date(crrentDay.getFullYear(), crrentDay.getMonth(),
				0).getDate();

		var firstIndex = new Date(crrentDay.getFullYear(),
				crrentDay.getMonth() - 1, 1).getDay();
		if (firstIndex == 0) {
			firstIndex = 6;
		} else {
			firstIndex -= 1;
		...
		for (var j = 0; j < 6; j++) {
			...
			for (var i = j * 7; i < (j + 1) * 7; i++) {
				...
				if (days[i] == "&nbsp;") {
					...
					newTd.innerHTML = "<font size='5' style='color: #149ac1;cursor:pointer'>"
							+ days[i] + "</font>";
				}
							...
							<tr>
								<td>
									<table border="0" cellspacing="0" cellpadding="0"
										...
										<tr>
											<td colspan="7" align="center" height="50"><select
												id="year" onchange="showCalender();">
											...
											</select>&nbsp;年&nbsp; <select id="month" onchange="showCalender();">
										...
										<tr>
											...
											<th width="100px" height="60px"><font size="size="5">一</font>
											</th>
											<th width="100px" height="60px"><font size="size="5">二</font>
											</th>
											<th width="100px" height="60px"><font size="size="5">三</font>
											</th>
											<th width="100px" height="60px"><font size="size="5">四</font>
											</th>
											<th width="100px" height="60px"><font size="size="5">五</font>
											</th>
											<th width="100px" height="60px"><font color="red"
												size="5">六</font></th>
											<th width="100px" height="60px"><font color="red"
												size="5">日</font></th>
										</tr>
			...
			<table border="1px" cellpadding="0" cellspacing="0"
				...
				<c:forEach items="${sessionScope.departments}" var="departments">
					<tr>
						...
						<td>${departments.number}</td>
						<td><a href="updateDepartment?id=${departments.id}" class = "update">EDIT</a></td>
						<td><a href="deleteDepartment?id=${departments.id}" class ="del">DELETE</a></td>
						<td><a href="queryDetail?id=${departments.id}">DETAIL</a></td>
					</tr>
				</c:forEach>
			</table>
		</div>
