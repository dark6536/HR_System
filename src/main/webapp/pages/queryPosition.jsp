		...
		<div id = "div_right">
			<table border="1px" cellpadding="0" cellspacing="0" class = "table_query">
				...
				<tr>
					...
					<td>${positions.department.name}</td>
					<td>${positions.powerType}</td>
					<td>${positions.createTime}</td>
					<td><a href = "updatePosition?id=${positions.id}" class = "update">EDIT</a></td>
					<td><a href = "deletePosition?id=${positions.id}" class = "del">DELETE</a></td>
					<td><a href = "queryDetail?id=${positions.id}">DETAIL</a></td>
				</tr>
				</c:forEach>	
			</table>
		</div>
